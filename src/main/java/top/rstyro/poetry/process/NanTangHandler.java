...
@Slf4j
@Component
public class NanTangHandler implements BaseHandler{
    ...
    public void handler(String filePath) {
        ...
        Arrays.stream(files).filter(i->i.getName().endsWith(".json") && i.getName().contains("poetrys")).forEach(f -> {
            try {
    ...
    public void batchSavePoetry(List<PoetryHuaJianJiVo> list, CountDownLatch countDownLatch) {
        try {
            HashSet<String> dynasty = new HashSet<>(Arrays.asList("南唐"));
            HashSet<String> type = new HashSet<>(Arrays.asList("南唐二主词"));
            HashSet<String> tags = new HashSet<>(Arrays.asList("南唐二主词"));
