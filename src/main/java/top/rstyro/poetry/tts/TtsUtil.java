...
public class TtsUtil {
    ...
    public static InputStream getTtsInputStream(String content) {
        for (int i = 0; i < 100; i++) {
            try {
                String url = "https://southeastasia.api.speech.microsoft.com/accfreetrial/texttospeech/acc/v3.0-beta1/vcg/speak";
                String ssmlFormat = "<speak xmlns=\"http://www.w3.org/2001/10/synthesis\" xmlns:mstts=\"http://www.w3.org/2001/mstts\" xmlns:emo=\"http://www.w3.org/2009/10/emotionml\" version=\"1.0\" xml:lang=\"en-US\"><voice name=\"zh-CN-XiaoxiaoNeural\"><mstts:express-as  style=\"Default\" ><prosody rate=\"0%\" pitch=\"0%\">" +
                        content + "</prosody></mstts:express-as></voice></speak>";
                ParamDto paramDto = new ParamDto();
                paramDto.setSsml(ssmlFormat);
                HttpResponse response = HttpRequest.post(url).body(JSON.toJSONString(paramDto))
                        .header("authority", "southeastasia.api.speech.microsoft.com")
                        .header("accept", "*/*")
                        .header("accept-language", "zh-CN,zh;q=0.9")
                        .header("customvoiceconnectionid", IdUtil.simpleUUID())
                        .header("origin", "https://speech.microsoft.com")
        ...
        throw new RuntimeException("尝试获取tts流失败，请稍后尝试");
    }

    /**
     * 这个方法不行，AudioSystem.getAudioInputStream(tempfile) 报错，unsupported format
     *
     * @param mp3FileName
     * @param content
     * @return
